...
export function RecordsProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const savedOverride = window.localStorage.getItem(themeOverrideKey);
    if (savedOverride) {
      setThemeOverrideName(savedOverride);
    }
  }, []);
  ...
  const value = useMemo<RecordsContextValue>(
    () => ({
      records,
      today,
      isThemeOverridden: Boolean(overriddenTheme),
      setTodayThemeOverride: (themeName) => {
        window.localStorage.setItem(themeOverrideKey, themeName);
        setThemeOverrideName(themeName);
      },
      clearTodayThemeOverride: () => {
        window.localStorage.removeItem(themeOverrideKey);
        setThemeOverrideName(null);
      },
      currentStreak,
      monthlyCount,
      addRecord: (draft) => {
    ...
    [records, today, overriddenTheme, currentStreak, monthlyCount]
  );
